 ...
 */
@Slf4j
public class ConflictResolutionAgentImpl implements ConflictResolutionAgent {
    ...
    private String resolveTemporalConflict(Artifact a1, Artifact a2) {
        ...
        if (a1Consistent != a2Consistent) {
            return a1Consistent ? a1.getArtifactId() : a2.getArtifactId();
        }
        return resolveByRecency(a1, a2);
    }
    ...
    public String getDescription() {
        return "Detects and resolves contradictory memory artifacts and tracks belief supersession.";
    }
    ...
    public boolean canHandle(Task task) {
        ...
        return "CONFLICT".equalsIgnoreCase(task.getAgent())
                || getName().equalsIgnoreCase(task.getAgent())
                || objective.contains("conflict");
    }
    ...
    public CompletableFuture<List<Artifact>> executeTask(Task task) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                List<Contradiction> contradictions = detectContradictions(task.getInputs());
                List<Artifact> artifacts = contradictions.stream()
                        .map(this::toArtifact)
                        .toList();
                task.markComplete(artifacts.stream().map(Artifact::getArtifactId).toList());
                return artifacts;
            } catch (Exception e) {
    ...
    private Artifact toArtifact(Contradiction contradiction) {
        return new Artifact.Builder()
                .artifactId("conflict-" + contradiction.contradictionId)
                .producedBy(getName())
                .type(Artifact.ArtifactType.CONFLICT)
                .contentText(contradiction.description)
                .contentType("conflict")
                .metadata(Map.of(
                        "artifact1", contradiction.artifact1,
                        "artifact2", contradiction.artifact2,
    ...
    public void handleFailure(Task task, Exception error) {
        log.error("Conflict resolution task failed: {}", task == null ? "<null>" : task.getId(), error);
    ...
    public AgentMetrics getMetrics() {
        long total = contradictionsDetected;
        double successRate = total == 0 ? 1.0 : (double) successfulResolutions / total;
        return new AgentMetrics((int) successfulResolutions,
                (int) Math.max(0, total - successfulResolutions), 0L, successRate, 0L);
    }
